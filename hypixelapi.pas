...
var
  ...
  username,apikey,guildLVL,networkl: string;
   responses1,responses2,responses3,responses4,responseapikeyvalid: AnsiString;
   ...
   arrays: array [1..5] of TJSONArray;
...
begin
   if fileexists(filename) = FALSE then
    ...
    with tfphttpclient.create(nil) do
     ...
     arrays[5]:=TJSONArray(GetJSON(responseapikeyvalid));
     if arrays[5].FindPath('success') = nil then begin end;
     apikeyvalid:=utf8tostring(arrays[5].FindPath('success').AsString);
    ...
    if checkbox1.Checked = TRUE then
      showmessage('Successfully got data from Mojang API and Hypixel API.');
    // scout mojang api and the hypixel api for data
    ...
    for enums1 in arrays[3].FindPath('guild.members') do begin
       ...
       if (gRank = 'Guild Master') or (gRank = 'GUILDMASTER') then begin
         ...
         with tfphttpclient.create(nil) do
           try
             addheader('Content-Type','application/json');
             responses4:=get(concat('https://api.hypixel.net/player?key=',apikey,'&uuid=',gmasteruuid));
           finally
             free;
           end;
         arrays[4]:=TJSONArray(GetJSON(responses4));
         gmaster.caption:=utf8tostring(arrays[4].FindPath('player.displayname').asString);
       end;
    end;
           if checkbox1.Checked = TRUE then
    ...
    handler:=1;
    if arrays[2].FindPath('player.stats.SkyBlock.profiles') = nil then
    else
    for enums2 in arrays[2].FindPath('player.stats.SkyBlock.profiles') do begin
